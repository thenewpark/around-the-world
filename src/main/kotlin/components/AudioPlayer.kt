...
class VorbisTrack(filePath: String) {
    ...
    init {
        ...
        MemoryStack.stackPush().use { stack ->
            ...
            handle = STBVorbis.stb_vorbis_open_memory(encodedAudio ?: error("no audio"), error, null)
    ...
    fun setPosition(timeInSeconds: Double) {
        seek((timeInSeconds * sampleRate).toInt())
    }

    // called from audio thread
    @Synchronized
    fun getSamples(pcm: ShortBuffer?): Int {
        return STBVorbis.stb_vorbis_get_samples_short_interleaved(handle, channels, pcm ?: error("no pcm buffer"))
    }
...
//        }
    ...
    fun pause() {
        ...
        if (error != AL10.AL_NO_ERROR) {
            val msg = when (error) {
                AL_INVALID_OPERATION -> "AL_INVALID_OPERATION: There is no current context. "
                else -> null
            }
            logger.error { msg }
        }
...
fun loadAudio(filename: String): VorbisTrack {
    val f = File(filename)
    require(f.exists()) { "file '${filename}' does not exist"}
    return VorbisTrack(filename)
}